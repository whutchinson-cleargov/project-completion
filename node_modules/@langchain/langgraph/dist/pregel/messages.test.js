...
describe("StreamMessagesHandler", () => {
    ...
    describe("handleChatModelStart", () => {
        ...
        it("should not store metadata when TAG_NOSTREAM is present", () => {
            ...
            handler.handleChatModelStart({}, [], runId, undefined, {}, [TAG_NOSTREAM], // nostream tag
            metadata, "ModelName");
    ...
    describe("handleLLMNewToken", () => {
        it("should emit message chunk when metadata exists", () => {
            ...
            expect(emitSpy).toHaveBeenCalledWith(handler.metadatas[runId], expect.any(AIMessageChunk), runId);
        ...
        it("should emit provided chunk when available", () => {
            ...
            expect(emitSpy).toHaveBeenCalledWith(handler.metadatas[runId], chunk.message, runId);
        });
    ...
    describe("handleLLMEnd", () => {
        it("should emit message from non-streaming run", () => {
            ...
            expect(handler._emit).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ content: "final result" }), runId, true);
            // Should clean up
            expect(handler.metadatas[runId]).toBeUndefined();
        });
    ...
    describe("handleChainStart", () => {
        ...
        it("should not store metadata when TAG_HIDDEN is present", () => {
            ...
            handler.handleChainStart({}, {}, runId, undefined, [TAG_HIDDEN], // Hidden tag
    ...
    describe("handleChainEnd", () => {
        it("should emit a single message output", () => {
            ...
            // Should emit the message with dedupe=true
            expect(handler._emit).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ content: "chain result" }), runId, true);
            // Should clean up
            expect(handler.metadatas[runId]).toBeUndefined();
        });
